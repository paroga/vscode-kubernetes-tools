...
function validatePortMapping(portMapping: string, validPorts: ExtractedPort[] = []): ValidationResult | undefined {
    ...
    const validationResults: ValidationResult[] = portPairs.map(validatePortPair.bind(null, validPorts));
...
function validatePortPair(validPorts: ExtractedPort[], portPair: string): ValidationResult {
    ...
    if (!portPair.includes(':') && isPortValid(validPorts, portPair)) {
        return {
            valid: true
        };
    }
    ...
    if (
        isPortValid(validPorts, localPort, true) &&
        isPortValid(validPorts, targetPort)
    ) {
        return {
            valid: true
        };
 ...
 * @param nullable Can the port be empty or zero
 * @returns Boolean identifying if the port is valid
 */
function isPortValid(validPorts: ExtractedPort[], port: string, nullable?: boolean): boolean {
    if (validPorts.map(({name}) => name).includes(port)) {
        return true;
    }
    if (nullable && ['', '0'].includes(port)) {
        return true;
    }
 ...
 */
export function buildPortMapping(portString: string, validPorts: ExtractedPort[] = []): PortMapping[] {
    const portPairs = portString.split(' ');
    return portPairs.map(buildPortPair.bind(null, validPorts));
}
...
function buildPort(validPorts: ExtractedPort[], port: string): number {
    const found = validPorts.filter(({name}) => name === port);
    if (found.length > 0) {
        return found[0].port;
    }
    return Number(port);
